...
Public Class PALLET_CREATE
    ...
    Private Sub GRIDDATABIND()
        Try
            ...
            APPCON.CNN.Open()
            Dim STRSQL As String
            STRSQL = "SELECT * FROM PACKING WHERE ITEMCODE ='" & Trim(MCODE.Text) & "' and trno='" & Trim(L) & "'AND trno NOT IN (SELECT BARCODE FROM PALLET WHERE ITEMCODE ='" & Trim(MCODE.Text) & "' and barcode='" & Trim(L) & "')"
    ...
    Private Sub INSERTDATA()
        Try
            ...
            If Trim(Grid1.Cell(1, 1).Text) <> "" Then
                StrSql = StrSql & "truncate table temp_PALLET "
                StrSql = StrSql & "INSERT INTO [temp_PALLET]([ITEMNAME],[ITEMCODE],[BARCODE],[date],[time],USERNAME,ID,FINYEAR,MQTY,[TRNO])" _
                              & " values (" _
            ...
            Grid1.Rows = 2
            System.Diagnostics.Process.Start(Application.StartupPath + "\PALLET.bat")
            MsgBox("SAVED")
        Catch ex As Exception
        ...
        Try
            ...
            If Val(CMBQTY.Text) = Val(TXTCOUNTER.Text) Then
                ...
                If CMBINAME.Text = "" Then
                    ...
                    Exit Sub
                End If
                BILLFIRSTWORD = GENFUNCTION.GETFIRSTWORDCAPTION("PALLET", PmStoreDESC, pmFinYear)
                BILLNO = GENFUNCTION.AutoCodeGen("PALLET", "TRNO", BILLFIRSTWORD, "", "10", "")
